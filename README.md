...
├── data/                    # runtime: sessions/<id>.db + <id>/MD/ (копии контента сессии)
├── MD/                      # ★ редактируемый контент: промпты / листы / сеттинги + ready.md
...
## MD-хранилище и ready.md (редактируемый контент)

Весь текстовый контент сведён в `MD/` (см. `MD/README.md`):

```
MD/prompts/*.md      # системные промпты ({{EDITION_LABEL}}/{{EDITION_NOTE}} подставляются сами)
MD/characters/*.md   # листы персонажей (сюда падают и загрузки игроков)
MD/settings/*.md     # жанровые сеттинги (registry.json — служебный каталог)
MD/ready.md          # переключатель коммита: False → True
```

Как применить правки:

1. Отредактируй любой файл в `MD/`.
2. Запиши `True` в `MD/ready.md`.
3. После полного завершения текущего раунда бот перенесёт правки
   во внутренний кэш `MD/_applied/` и вернёт флаг в `False`.
4. Увидят их только НОВЫЕ сессии: при создании сессии снимается
   слепок в `data/sessions/<id>/MD/.../*_copy.md`, и движок всю жизнь
   сессии читает ТОЛЬКО его. Уже идущие сессии доигрывают со своей
   копией — это осознанное поведение. Копия удаляется при завершении
...
Юнит-тесты живут в `tests/` (pytest + pytest-asyncio, без сети/Telegram/SQLite):

```bash
python -m pytest tests/ -v          # весь набор (~2 сек)
python -m pytest tests/test_validators.py -v   # один файл
```

Покрыто: валидация листов (`validators.py` + `libs/services/character_validation.py`),
парсер листов (`db_bot_engine.py`), счётчик раундов и очередь действий
(`round_coordinator.py`), боевая механика (`combat_coordinator.py`),
пакет `/creu` (dice/formatter/storage).

CI: `.github/workflows/tests.yml` — pytest на Python 3.11/3.12 при каждом push/PR.

Архитектурный процесс (рекомендация код-ревью): доменная логика постепенно
выносится из миксинов в сервисные классы с явными зависимостями
(`libs/services/`); `CharacterValidationService` — первый пример, миксин
`ValidatorsMixin` остался тонким back-compat адаптером.
