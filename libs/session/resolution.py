...
class ResolutionMixin:
    ...
    async def resolve_round_master_only(self, session_id: str, roll_mode: str = "mixed",
        ...
        БОЙ-FIX: всё тело выполняется под session-wide резолвер-мьютексом
        (get_resolver_lock) — обычный раунд больше не может пересекаться с
        боевой циклом/NPC-ходом/dual-narrative. Пока этот раунд резолвится,
        остальные пути ждут; их действия копятся в очередях и разрешаются
        следующими — без дублей, без «воскресших» NPC, без скачков раундов.
        """
        async with self.get_resolver_lock(session_id):
            return await self._resolve_round_master_only_locked(
                session_id, roll_mode=roll_mode, player_roll_requester=player_roll_requester)

    async def _resolve_round_master_only_locked(self, session_id: str, roll_mode: str = "mixed",
                            player_roll_requester=None) -> Dict[str, str]:
        """Implementation of resolve_round_master_only — MUST be called while holding
        the session resolver lock (see get_resolver_lock)."""
        ...
        try:
            ...
            # БОЙ-FIX: остатки non-combat очереди (dual-narrative), оставшиеся
            # после конца боя, подмешиваем к первому же обычному раунду —
            # раньше end_initiative_combat их просто терял. Если игрок уже
            # подал действие в основной раунд — берём его (новее), остаток скипаем.
            # БОЙ-FIX v2: подмешиваем ТОЛЬКО когда бой уже закончился — пока
            # combat_active, non-combat очередь принадлежит dual-narrative lane;
            # иначе обычный раунд утащит действия игроков из параллельной линии
            # и напишет вторую версию сцены («нейросеть разрешает, а не ждёт»).
            session_pre = db.get_session(session_id)
            if session_pre and session_pre.combat_active:
                leftovers = {}
            else:
                try:
                    leftovers = self.pop_non_combat_queue(session_id) or {}
                except Exception:
                    leftovers = {}
            for left_id, left_action in leftovers.items():
                if left_id in nick_by_player_id:
                    continue
                left_player = db.get_player(left_id, session_id)
                if not left_player:
                    continue
                left_nick = left_player.username or left_player.display_name
                player_actions[left_nick] = f"Дн. {left_action}"
                nick_by_player_id[left_id] = left_nick
            ...
            # Фича 3: номер текущего раунда — инкрементируется на каждый нарративный
            # раунд и попадает в контекст Мастера + в метки истории [Раунд N].
            round_number = self.advance_round(session_id)

            # Build context
            context_lines = []
            session = db.get_session(session_id)

            context_lines.append(f"--- СЕЙЧАС ИДЁТ РАУНД {round_number} игры (счётчик всех нарративных раундов; история помечена метками [Раунд N]) ---")
            ...
            result = await self.dm.process_master_turn(
                ...
                # ДОПОЛНЕНИЕ 1: скрытый канал состояния НПС (HP/состояния/заметки)
                npc_state_syncer=lambda payload: self._npc_state_syncer_async(session_id, payload),
                npc_ai_enabled=True,
            ...
            # Save to history (Фича 3: метки [Раунд N] — по ним нейросеть отвечает
            # на вопросы «что я делал N раундов назад» и не выдумывает прошлое)
            for nick, full_action in player_actions.items():
                db.add_history(HistoryEntry(
                    session_id=session_id,
                    author=nick,
                    content=f"[Раунд {round_number}] {full_action}",
                    entry_type="action",
                ))
            ...
            db.add_history(HistoryEntry(
                session_id=session_id,
                author="DM",
                content=f"[Раунд {round_number}] {player_text}",
            ...
            if gm_log:
                db.add_history(HistoryEntry(
                    session_id=session_id,
                    author="GM_SECRET",
                    content=f"[Раунд {round_number}] {gm_log}",
    ...
    async def resolve_non_combat_round(self, session_id: str, roll_mode: str = "mixed",
        ...
        This runs alongside the combat turn loop, so non-combat players don't wait
        ...
        БОЙ-FIX: «параллельно» больше НЕ значит «одновременно». Всё тело
        выполняется под session-wide резолвер-мьютексом: пока Мастер занят
        боевым ходом (NPC или PC), non-combat раунд ждёт своей очереди и
        разрешается сразу после — действия за это время копятся в очереди
        (submit_non_combat_action) и разрешаются одним пакетом. Это убирает
        именно ту гонку, которая давала два противоречащих нарратива, два
        набора бросков и два расходящихся состояния HP на одну сцену.
        """
        async with self.get_resolver_lock(session_id):
            return await self._resolve_non_combat_round_locked(
                session_id, roll_mode=roll_mode, player_roll_requester=player_roll_requester)

    async def _resolve_non_combat_round_locked(self, session_id: str, roll_mode: str = "mixed",
                                        player_roll_requester=None) -> Dict[str, str]:
        """Implementation of resolve_non_combat_round — MUST be called while holding
        the session resolver lock (see get_resolver_lock)."""
        ...
        # Фича 3: номер раунда для параллельного нон-комбат-нарратива
        round_number = self.advance_round(session_id)
        ...
        session = db.get_session(session_id)
        context_lines = [f"--- СЕЙЧАС ИДЁТ РАУНД {round_number} игры (счётчик всех нарративных раундов; история помечена метками [Раунд N]) ---",
                         "⚠️ ВНИМАНИЕ: Параллельно идёт БОЙ с другими персонажами. Эти игроки НЕ в бою — "
                         "пиши нарратив только для них. Бой происходит отдельно и одновременно.",
                         "⚔️ ЕСЛИ действие этого игрока фактически вмешивается в идущий бой (атакует участников боя, "
                         "вступает в схватку, прикрывает кого-то от врагов) — СНАЧАЛА вызови инструмент ymuno_ymladd "
                         "с именем его персонажа, чтобы система добавила его в инициативу, и пиши нарратив как "
                         "вступление в бой. Его боевые броски будут запрошены на ЕГО ходу через request_player_roll. "
                         "Если действие НЕ касается боя — не вызывай ymuno_ymladd и пиши обычный нарратив."]
        if chars:
            context_lines.append("\n--- СТАТУС ПЕРСОНАЖЕЙ ---")
            for c in chars:
                status = "💀" if not c.is_alive else f"❤️ {c.hp}/{c.max_hp} HP"
        ...
        result = await self.dm.process_master_turn(
            ...
            # БОЙ-FIX v2: НО вступить в УЖЕ ИДУЩИЙ бой игрок может — это ровно
            # тот случай «Рими бежит к слизи с ударами», который раньше
            # резолвился как параллельная сцена с расходящимся состоянием.
            combat_joiner=lambda name: self._combat_joiner_async(session_id, name),
            # ДОПОЛНЕНИЕ 1: скрытый канал работает и вне боя (записная книжка НПС)
            npc_state_syncer=lambda payload: self._npc_state_syncer_async(session_id, payload),
        ...
        # Save to history (Фича 3: метки [Раунд N])
        for nick, full_action in player_actions.items():
            db.add_history(HistoryEntry(
                session_id=session_id,
                author=nick,
    ...
    def _apply_game_actions(self, session_id: str, actions: List[Dict]) -> Tuple[int, List[str]]:
        ...
        # ДОПОЛНЕНИЕ 1 (скрытый канал Мастер → DB): если смена HP/состояния
        # приходит на имя, которого нет среди персонажей игроков, — это НПС.
        # В бою обновляем Combatant активного encounter СКРЫТНО (никакого
        # вывода в чат), чтобы боевой контекст Мастера всегда видел актуальное
        # HP/состояния НПС и мог понять, что бой окончен (Баг 3).
        def find_npc_combatant(name: str):
            ...
            name_lower = name.lower()
            for c in combatants:
                if c.entity_type != "pc" and c.name.lower() == name_lower:
                    return c
            for c in combatants:
                if c.entity_type != "pc" and (
                    name_lower in c.name.lower() or c.name.lower() in name_lower
                ):
                    return c
            return None

        def _combatant_conditions(comb) -> List[str]:
            try:
        ...
        for act in actions:
            ...
            try:
                if tool == "change_hp":
                    ...
                    if not char:
                        # ДОПОЛНЕНИЕ 1: это НПС — скрыто обновляем Combatant
                        comb = find_npc_combatant(args.get("character_name", ""))
                        if comb:
                            ...
                            try:
                                ...
                                delta = 0
                            max_hp = comb.max_hp or comb.hp
                            new_hp = max(0, min(comb.hp + delta, max_hp)) if max_hp else max(0, comb.hp + delta)
                            updates = {"hp": new_hp}
                            if new_hp <= 0:
                                updates["is_alive"] = False
                            db.update_combatant(comb.id, **updates)
                            logger.info(f"[npc-hidden-sync] change_hp NPC '{comb.name}': {comb.hp} -> {new_hp} ({args.get('source', 'AI')})")
                            applied += 1
                            continue
                    ...
                    char = find_char(args.get("character_name", ""))
                    if not char:
                        # ДОПОЛНЕНИЕ 1: состояние НПС — скрыто в Combatant
                        comb = find_npc_combatant(args.get("character_name", ""))
                        if comb:
                            cond = args.get("condition", "").lower()
                            if cond:
                                conds = _combatant_conditions(comb)
                                if cond not in conds:
                                    conds.append(cond)
                                db.update_combatant(comb.id, current_conditions=json.dumps(conds, ensure_ascii=False))
                                logger.info(f"[npc-hidden-sync] add_condition NPC '{comb.name}': {cond}")
                                applied += 1
                            continue
                    ...
                    char = find_char(args.get("character_name", ""))
                    if not char:
                        # ДОПОЛНЕНИЕ 1: снятие состояния НПС — скрыто в Combatant
                        comb = find_npc_combatant(args.get("character_name", ""))
                        if comb:
                            cond = args.get("condition", "").lower()
                            conds = [c for c in _combatant_conditions(comb) if c != cond]
                            db.update_combatant(comb.id, current_conditions=json.dumps(conds, ensure_ascii=False))
                            logger.info(f"[npc-hidden-sync] remove_condition NPC '{comb.name}': {cond}")
                            applied += 1
                            continue
    ...
    async def answer_question(self, session_id: str, player_id: int,
        ...
        answer = await self.dm.answer_question(history_msgs, question, name, session_id=session_id)

        # Фича 3: помечаем OOC-запись текущим раундом (без инкремента)
        _round = self.get_current_round(session_id)
        _round_tag = f"[Раунд {_round}] " if _round else ""
        db.add_history(HistoryEntry(
            session_id=session_id,
            author=name,
            content=f"{_round_tag}[OOC Question]: {question}",
            entry_type="ooc",
        ))
        db.add_history(HistoryEntry(
            session_id=session_id,
            author="DM",
            content=f"{_round_tag}[OOC Answer]: {answer}",
            entry_type="ooc",
        ))
