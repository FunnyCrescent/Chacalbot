...
from .prompts import MODER_AI_DISPATCH_PROMPT, MASTER_PROMPT
    ...
    async def moder_ai_dispatch_roll(
        ...
        try:
            response = await self.moder_ai.chat(
                messages,
                system_prompt=MODER_AI_DISPATCH_PROMPT,
    ...
    def _fetch_character_sheet_by_username(self, username: str, session_id: str) -> str:
        ...
        for p in players:
            if p.username and p.username.lower() == username_lower:
                target_player = p
                break
            # Fallback: partial match (sometimes stored without leading @)
            if p.username and username_lower in p.username.lower():
                target_player = p
                break
