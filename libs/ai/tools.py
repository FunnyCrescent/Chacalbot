...
COMBAT_TOOLS = [
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "priorities": {
                        "type": "object",
                        "description": (
                            "НЕОБЯЗАТЕЛЬНО — тай-брейк инициативы при РАВНЫХ значениях. "
                            "Ключ — имя участника, значение — целое число; МЕНЬШЕ = ходит РАНЬШЕ. "
                            "Указывай ТОЛЬКО тех участников, чья инициатива реально одинакова, "
                            "и ТОЛЬКО если ты уже написал(а) в нарративе, кто из них ходит первым — "
                            "поле фиксирует твоё слово, чтобы система его не потеряла при сортировке. "
                            "Игроки видят сырую инициативу («оба 15») — priority в показ не входит."
                        ),
    ...
    },
    {
        ...
        "function": {
            "name": "ymuno_ymladd",
            "description": (
                "Добавить ПЕРСОНАЖА-ИГРОКА в УЖЕ ИДУЩИЙ бой. Вызывай, когда бой начат, а "
                "персонаж, не числившийся участником, по своему Дн. фактически вмешивается в него: "
                "атакует участников боя, вступает в схватку, прикрывает союзников от врагов. "
                "Система бросит ему инициативу и вставит в порядок ходов — его боевые броски "
                "запрашивай через request_player_roll уже на ЕГО ходу. НЕ вызывай для NPC/монстров "
                "(они добавляются через dechrauymladd) и не вызывай, если персонаж уже в бою. "
                "Если действие игрока НЕ вмешивается в бой (торгует, смотрит в стороне, в другой локации) "
                "— НЕ вызывай, просто пиши нарратив для него как обычно."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "character_name": {
                        "type": "string",
                        "description": "Имя персонажа-игрока, вступающего в идущий бой.",
                    },
                    "reason": {
                        "type": "string",
                        "description": "Краткая причина вступления в бой, для лога.",
                    },
                },
    ...
    },
    {
        ...
        "function": {
            "name": "diweddaru_npc",
            "description": (
                "СКРЫТЫЙ КАНАЛ К DB-БОТУ: сохранить актуальное состояние НПС/монстра "
                "(HP, состояния, важные заметки). Игроки НЕ видят этот вызов и его "
                "результат — только ты и система. Вызывай КАЖДЫЙ раз, когда НПС "
                "получил урон, лечение, новое состояние (сбит с ног, испуган, отравлен) "
                "или изменился важный факт о нём — сразу после разрешения хода, ДО "
                "финального нарратива. Из этого состояния ты сам будешь видеть HP НПС "
                "в боевой сводке на следующих ходах. Игрокам раскрывай HP/состояние НПС "
                "ТОЛЬКО если они законно узнали это в игре: успешная проверка "
                "Внимательности, Расследования, Медицины, Анализа и т.п., или явное "
                "сюжетное событие."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "npc_name": {
                        "type": "string",
                        "description": "Имя НПС/монстра как в бою или в мире.",
                    },
                    "hp_current": {
                        "type": "integer",
                        "description": "Текущее HP после изменений. Передавай ВСЕГДА, когда менялось HP.",
                    },
                    "hp_max": {
                        "type": "integer",
                        "description": "Максимум HP (если известен). Не меняй без необходимости.",
                    },
                    "conditions": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Актуальный СПИСОК состояний целиком (сбит с ног, испуган, отравлен...). Пустой массив = состояний нет.",
                    },
                    "is_alive": {
                        "type": "boolean",
                        "description": "false, если НПС мёртв или без сознания окончательно.",
                    },
                    "note": {
                        "type": "string",
                        "description": "Любая важная СЕКРЕТНАЯ информация о НПС для себя и DB-бота: слабости, намерения, что игроки не знают.",
                    },
                    "revealed": {
                        "type": "boolean",
                        "description": "true, если игроки УЖЕ законно узнали HP/состояние этого НПС через навык (Внимательность, Расследование, Медицина) — тогда это больше не секрет.",
                    },
                },
                "required": ["npc_name"],
            },
    ...
    },
    {
        ...
        "function": {
            "name": "diweddymladd",
            "description": "Завершить бой. Вызывай когда все враги мертвы, сбежали, сдались, или бой окончен по сюжету.",
            "parameters": {
                "type": "object",
                "properties": {
