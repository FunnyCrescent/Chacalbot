...
from libs.ai_client import DMEngine, MASTER_PROMPT, MASTER_TOOLS, OpenAIClient, get_prompt
...
from libs.handlers._state import (
    ...
    mark_world_generating, clear_world_generating, is_world_generating,
)
...
async def _process_character_upload(update: Update, ctx: ContextTypes.DEFAULT_TYPE,
    ...
    try:
        ...
        # BUG FIX (Эйра «Чертила»): an EMPTY file used to sail through validation,
        # the parser hallucinated a character from its own prompt template (which
        # contained a real player's sheet as an example) and it got SAVED as this
        # player's character. Reject empty/near-empty files immediately.
        if len(re.findall(r"[A-Za-zА-Яа-яЁё0-9]", sheet_text or "")) < 10:
            await send_safe(update,
                "❌ **Файл пустой или не содержит текста.**\n\n"
                "Прикрепи настоящий лист персонажа: `.txt` или `.md` с именем, расой, классом, характеристиками."
            )
            return
        ...
        if validation["verdict"] == "REJECT":
            ...
            # UNKNOWN / ERROR — the validator didn't give a clear verdict.
            # Previously these verdicts fell through and the upload CONTINUED —
            # an unchecked sheet could be saved. Now we stop and tell the user.
            await send_safe(update,
                f"❓ **Не удалось проверить лист (вердикт: {validation['verdict']}).**\n\n"
                f"{str(validation.get('details', ''))[:600]}\n\n"
                f"🔧 Попробуй загрузить ещё раз командой `/cymeriad`."
            )
            return
        ...
        if not parsed:
            ...
            await send_safe(update,
                ...
                "Проверь файл и загрузи заново командой `/cymeriad`."
            )
            return
...
async def delete_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """BUG #7 + #10: Delete all session data AND fully stop the game.

    Previously /dileu only flipped the session status to 'ended' and cleared the
    history rows — but the .db file stayed, the in-memory combat loop kept running,
    /ailddechrau could "resume" the deleted session, and pending rolls/deferred
    resolves still fired. After this fix:

    1. clear_session_runtime_state — drops in-memory per-session caches (pending
       rolls, deferred resolves, db_busy flag, round locks, non-combat queue).
    2. db_manager.end_session — flips the session row to status='ended' and clears
       queue_state. Also pops chat_id from the in-memory _chat_to_session cache so
       `get_session(chat_id)` returns None — every command handler that checks
       `if not session: return` will now refuse all subsequent rolls/actions.
    3. db_manager.drop_session — physically unlinks the .db file (and its
       -wal/-shm sidecars) so /ailddechrau cannot resurrect it. This is the
       GDPR-style "right to be forgotten" the user-facing message already promised
       but the code never actually delivered.

    All combat loop / DB-Bot background tasks that were still referencing the
    session will no-op: their next `db.get_session(session_id)` call returns None
    and they early-return. No further rolls, narratives, or HP changes can fire
    after /dileu returns. The chat is fully stopped until a new /dechrau creates a
    brand-new session."""
    chat_id = update.effective_chat.id
    user = update.effective_user
    session = get_session(chat_id)
    ...
    # BUG #10: drop in-memory per-session runtime state FIRST so any in-flight
    # async tasks (combat loop, deferred resolve, _auto_skip_turn) see no session
    # the next time they look.
    sessions.clear_session_runtime_state(session_id)

    # BUG #7: stop the combat loop guard so no new combat turn iteration can fire.
    _active_combat_loops.pop(session_id, None)

    # Remove the markdown export file (best-effort).
    export_path = os.path.join(BASE_DIR, "logs", f"session_{session_id}_{session_name.replace(' ', '_')}.txt")
    if os.path.exists(export_path):
        try:
            os.remove(export_path)
        except OSError:
            pass
    # Also remove the markdown journal written by MarkdownLogger.
    md_path = os.path.join(BASE_DIR, "logs", "markdown", f"session_{session_id}.md")
    ...
    # Flip status to 'ended' and clear queue_state. This also pops chat_id from
    # the in-memory _chat_to_session cache, so every `get_session(chat_id)` call
    # from this point on returns None — all command handlers early-return.
    db_manager.clear_history(session_id)
    db_manager.end_session(session_id)

    # BUG #7 + #10: physically delete the .db file so /ailddechrau (resume) cannot
    # resurrect the session. This is the "right to be forgotten" the user-facing
    # message already promised but never actually delivered before this fix.
    try:
        db_manager.drop_session(session_id)
    except Exception as e:
        logger.warning(f"[delete_cmd] drop_session failed for {session_id}: {e}")

    await send_safe(update, "🗑️ **Игра полностью остановлена.** Все данные сессии удалены: история, персонажи, мир, кеш, .db-файл. Восстановление через `/ailddechrau` невозможно — нужно создавать новую сессию через `/dechrau`.")
    if ADMIN_CHAT_ID and ctx:
        await send_to_admin(ctx, f"🗑️ Session deleted by user: {session_name} ({session_id})")
...
async def dndstart_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # BUG 3 FIX: защита от повторного запуска генерации (двойной /dndcychwyn,
    # /dndcychwyn во время genworld-колбэка и т.п.).
    if is_world_generating(session.id):
        await send_safe(update, "🌍 Генерация мира уже идёт — подожди, ходы начнутся автоматически.")
        return
    ...
    try:
        ...
        if not args_text and selected_genre:
            ...
            if setting_info:
                setting_content = _load_setting_md(selected_genre, session_id=session.id)
            ...
            if setting_info:
                setting_content = _load_setting_md(args_text.lower(), session_id=session.id)
        ...
        # BUG 3 FIX: поднимаем флаг «мир генерируется» — пока он поднят, Дн./rholio/
        # gofyn/gwneud у игроков отклоняются гейтом world_gen_guard.
        mark_world_generating(session.id, chat_id)
...
Отвечай на русском. Используй формат бросков если нужны скрытые проверки (visible=false)."""
        ...
        opening_resp = await dm_engine.master.chat(opening_msgs, system_prompt=get_prompt("master", session.id), tools=MASTER_TOOLS)
        ...
        # BUG 3 FIX: снимаем флаг генерации В ЛЮБОМ случае (даже при ошибке),
        # иначе сессия навсегда останется заблокированной для ходов.
        clear_world_generating(session.id)
        if typing_task:
            stop_typing.set()
            typing_task.cancel()
