...
async def hp_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await send_safe(update, "⚠️ Только просмотр. Изменения HP — через `Дн.` (нейросеть).")
    ...
    if arg.startswith("+") or arg.startswith("-"):
        try:
            ...
            await send_safe(update, "Использование: `/iechyd +5` или `/iechyd -10`")
            return
        ...
        if result["status"] == "dying":
            await send_safe(update, 
                ...
                f"`/arbedmarwolaeth {char.name}` — бросить\n"
                f"`/arbedmarwolaeth {char.name} success` — отметить успех\n"
                f"`/arbedmarwolaeth {char.name} failure` — отметить провал"
            )
    else:
        await send_safe(update, "Использование:\n`/iechyd` — показать\n`/iechyd +5` — лечить (только Админ)\n`/iechyd -10` — урон (только Админ)")
...
async def condition_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await send_safe(update, "⚠️ Только просмотр. Состояния — через `Дн.` (нейросеть).")
...
async def gold_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await send_safe(update, "⚠️ Только просмотр. Изменения золота — через `Дн.` (нейросеть).")
...
async def inventory_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await send_safe(update, "⚠️ Только просмотр. Изменения инвентаря — через `Дн.` (нейросеть).")
...
async def quest_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await send_safe(update, "⚠️ Только просмотр. Квесты — через `Дн.` (нейросеть).")
