...
def _load_setting_md(setting_id: str, session_id: str = None) -> Optional[str]:
    """Load the full .md content of a setting file. Returns None if not found.

    MD-консолидация (Фаза 2): содержимое сеттинга читается из ЗАМОРОЖЕННОЙ
    per-session копии data/sessions/<sid>/MD/settings/<stem>_copy.md.
    Фолбэк на источник (MD/settings/...): сеттинг добавлен в каталог ПОСЛЕ
    снапшота сессии (например, готовый коммит принёс новый жанр) — иначе
    движок не смог бы сгенерировать мир по новому сеттингу до пересоздания
    сессии. Имя файла резолвится по ГЛОБАЛЬНОМУ registry.json — это
    каталог-метаданные UI (/categori и т.п.), не игровой контент.
    """
    ...
    for cat_key, cat_data in registry.get("categories", {}).items():
        ...
        if file_name:
            break
    if file_name:
        # 1) per-session копия (единственный источник ВО ВРЕМЯ игры)
        if session_id:
            try:
                from libs.md_store import get_session_copy_path
                copy_path = get_session_copy_path(session_id, "settings", os.path.basename(file_name))
                if copy_path:
                    with open(copy_path, "r", encoding="utf-8") as f:
                        return f.read()
            except Exception as e:
                logger.warning(f"[md_store] setting copy read failed ({setting_id}/{session_id}): {e}")
        # 2) фолбэк: источник (сеттинг вне снапшота сессии / DM-режим / тесты)
    ...
    if session_id:
        try:
            from libs.md_store import get_session_copy_path
            copy_path = get_session_copy_path(session_id, "settings", f"custom/{setting_id}.md".split("/")[-1])
            if copy_path:
                with open(copy_path, "r", encoding="utf-8") as f:
                    return f.read()
        except Exception:
...
async def _categori_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "cat:root":
        ...
        try:
            ...
            # BUG 3 FIX: не запускаем вторую генерацию параллельно.
            if is_world_generating(session.id):
                await query.edit_message_text(
                    "🌍 Генерация мира уже идёт — подожди, ходы начнутся автоматически.",
                    parse_mode="HTML",
                )
                return
            ...
            # Build the theme with setting content (per-session копия — Фаза 2)
            setting_content = _load_setting_md(setting_id, session_id=session.id)
            theme = name
            if setting_content:
            ...
            # BUG 3 FIX: поднимаем флаг «мир генерируется» — Дн./rholio/gofyn у игроков
            # отклоняются, пока идёт генерация. Снимается в finally ниже.
...
Отвечай на русском. Используй формат бросков если нужны скрытые проверки (visible=false)."""
            ...
            # BUG #2: capture the forum topic thread id so /dndstart narratives
            # and the "Игра началась!" announcement go back to the same topic.
            thread_id = None
            if update and update.message and getattr(update.message, 'message_thread_id', None):
                thread_id = update.message.message_thread_id
            elif session and getattr(session, 'message_thread_id', 0):
                thread_id = int(session.message_thread_id)
            # Persist for async background sends (combat loop, DB-Bot phase, etc.).
            if session and thread_id and (session.message_thread_id or 0) != int(thread_id):
                try:
                    db = db_manager.get_db(session.id)
                    db.set_session_thread_id(session.id, int(thread_id))
                    session.message_thread_id = int(thread_id)
                except Exception:
                    pass
            if world_html.strip():
                world_converted = md_to_html(world_html)
                if world_converted.strip():
                    await _send_long_blockquote(bot_instance, chat_id, world_converted, message_thread_id=thread_id)

            # Send opening narrative as expandable (split long messages by paragraph)
            if opening_html.strip():
                await _send_long_blockquote(bot_instance, chat_id, opening_html, message_thread_id=thread_id)
            ...
            if tags_html:
                send_kwargs = {"chat_id": chat_id, "text": tags_html, "parse_mode": "HTML"}
                if thread_id:
                    send_kwargs["message_thread_id"] = thread_id
                await bot_instance.send_message(**send_kwargs)
            ...
            # BUG 3 FIX: снимаем флаг генерации в любом случае — иначе ходы навсегда заблокированы.
            try:
                _sid = session.id
            except Exception:
                _sid = None
            if _sid:
                clear_world_generating(_sid)
