...
async def private_action_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # BUG 3 FIX: приватные действия до/во время генерации мира запрещены.
    if await world_gen_guard(update, player_session):
        return
    ...
    try:
        ...
        try:
            ...
            if success:
                ...
                if session.chat_id:
                    try:
                        # BUG #2: route to the same forum topic thread where the
                        # private action was originally issued.
                        tid = getattr(session, 'message_thread_id', 0) or None
                        send_kwargs = {
