...
class SessionRepoMixin:
    ...
    def set_session_thread_id(self, session_id: str, message_thread_id: int):
        """BUG #2: persist the Telegram forum/topic thread id for this session so
        async/background sends (combat loop, DB-Bot phase, deferred resolves)
        can land back in the same topic where the player issued their command."""
        with self._connect() as conn:
            conn.execute(
                "UPDATE sessions SET message_thread_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                (int(message_thread_id or 0), session_id)
            )
