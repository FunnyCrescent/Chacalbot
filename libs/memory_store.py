...
# MD-консолидация: канонический текст промпта живёт в MD/prompts/consolidator.md;
# константа выше — фолбэк для случая, когда файл отсутствует (см. get_prompt).
def _consolidator_prompt(session_id: str = "") -> str:
    try:
        from libs.ai.prompts import get_prompt
        return get_prompt("consolidator", session_id)
    except Exception:
...
там ничего релевантного нет, ответь пустой строкой. Не выдумывай, не добавляй ничего от себя."""
    ...
    async def sleep_now(self, session_id: str) -> bool:
        ...
        try:
            ...
            response = await self.dm.memory.chat(messages, system_prompt=_consolidator_prompt(session_id), max_tokens=2000)
